...
def drawWmap(index, filename):
    ...
    m = Basemap(lon_0=180,
                projection='sinu', fix_aspect=True)
    
    # m.drawcoastlines()
    colorscheme = getcolor_scheme()
    i = random.randint(0, 10)
    m.fillcontinents(color=getColor(i, colorscheme))
    j = random.randint(0, 100)
    k = random.randint(30, 100)
    m.drawmapboundary(fill_color=Blues(j))
    m.drawcountries(linewidth=1, color = Greys(k))
    # m.drawlsmask(land_color='grey', ocean_color='b')
    ...
    img = original.transpose(Image.FLIP_LEFT_RIGHT)
...
def main():
    ...
    for i in range(0,60):
        ...
        filename = 'sinu_map_central_180_meridian_' + str(i) + '.png'
        if(i < 60):
            drawWmap(i, filename)
        # elif(i >= 15 and i < 30):
